...
void checkArgc(int argc, char *argv[]){
    if (argc != 5){
        perror("Usage : ./hw3named -i inputFilePath -n name\n");
    }else{
...
void chef(int chefIndex, char needs[2]){
    int counter = 0;
    printf("Chef%d (pid %d) is waiting for %s and %s\n", chefIndex, (int)getpid(), ingrediants(needs[0]), ingrediants(needs[1]));
    sleep(1);
    while (1){
        if (sig_check2())
            break;
...
void pusher1(){
    if (('W' == shr_ptr->ingred[0] || 'S' == shr_ptr->ingred[0]) && ('W' == shr_ptr->ingred[1] || 'S' == shr_ptr->ingred[1])){
        sem_post(needs_sems[0]);
    }else if (('F' == shr_ptr->ingred[0] || 'W' == shr_ptr->ingred[0]) && ('F' == shr_ptr->ingred[1] || 'W' == shr_ptr->ingred[1])){
        sem_post(needs_sems[1]);
    }else if (('F' == shr_ptr->ingred[0] || 'S' == shr_ptr->ingred[0]) && ('F' == shr_ptr->ingred[1] || 'S' == shr_ptr->ingred[1])){
        sem_post(needs_sems[2]);
    }else if (('F' == shr_ptr->ingred[0] || 'M' == shr_ptr->ingred[0]) && ('F' == shr_ptr->ingred[1] || 'M' == shr_ptr->ingred[1])){
        sem_post(needs_sems[3]);
    }else if (('M' == shr_ptr->ingred[0] || 'W' == shr_ptr->ingred[0]) && ('M' == shr_ptr->ingred[1] || 'W' == shr_ptr->ingred[1])){
        sem_post(needs_sems[4]);
    }else
        sem_post(needs_sems[5]);
}
char *ingrediants(char c){
    switch (c){
        case 'M':
